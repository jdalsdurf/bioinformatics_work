...
rule abricate_all:
    input:
        ...
        expand("abricateResults/{sample}_abricate_rtx_operon.csv", sample = config["samples"]),
        expand("abricateResults/{sample}_abricate_piliA_gene.csv", sample = config["samples"]),
...
rule run_all:
    ...
    output:
        ...
        rtx="abricateResults/{sample}_abricate_rtx_operon.csv",
        piliA="abricateResults/{sample}_abricate_piliA_gene.csv"
    ...
    shell:
        ...
        abricate {input} --csv --db {params.rtx} > {output.rtx}
        abricate {input} --csv --db {params.piliA} > {output.piliA}
        """
